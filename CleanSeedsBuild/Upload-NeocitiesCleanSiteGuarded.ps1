...
function Start-AttemptLog {
    param([string]$OutDir, [string]$LogPath)

    if ([string]::IsNullOrWhiteSpace($LogPath)) {
        $stamp = Get-Date -Format 'yyyyMMdd_HHmmss'
        $LogPath = Join-Path $OutDir "NEOCITIES_CLEAN_SITE_UPLOAD_ATTEMPT_$stamp.log"
    }

    $logDir = Split-Path -Path $LogPath -Parent
    if (-not (Test-Path -LiteralPath $logDir -PathType Container)) {
        throw "attempt log folder not found: $logDir"
    }

    try {
        Start-Transcript -Path $LogPath -Force | Out-Null
        $script:TranscriptStarted = $true
        $script:AttemptLogPath = $LogPath
        "attempt log: $LogPath"
    }
    catch {
        "attempt log unavailable: $($_.Exception.Message)"
    }
}
...
function New-UploadItem {
    ...
    [pscustomobject]@{
        ...
        SHA1 = (Get-FileHashHex -LiteralPath $local -AlgorithmName 'SHA1').ToLowerInvariant()
        SHA256 = Get-FileHashHex -LiteralPath $local -AlgorithmName 'SHA256'
    }
}
...
try {
    ...
    Start-AttemptLog -OutDir $OutDir -LogPath $LogPath
    ...
    if ($indexText -notmatch 'CLEAN SEED FILES' -or $indexText -notmatch 'Concept Maps' -or $indexText -notmatch 'Mom''s Seed Garden') {
        throw 'index.html does not contain the expected mom-front and public-reader markers'
    }
    ...
    try {
        ...
        if (-not [string]::IsNullOrWhiteSpace($script:AttemptLogPath)) {
            $receipt.Add("ATTEMPT_LOG: $script:AttemptLogPath")
        }
