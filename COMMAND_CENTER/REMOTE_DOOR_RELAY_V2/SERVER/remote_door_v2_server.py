...
rotates prints after accepted use, latches general proof into /door/v2/status,
and exposes receipt-bound proof through /door/v2/readback/<job_id>.
"""
...
class DoorState:
    ...
    def handle_print_probe(self, supplied_print: str, request_id: str, base_url: str) -> tuple[int, dict[str, Any]]:
        ...
        with self.print_lock:
            ...
            return 202, {
                ...
                "readback_url": f"{base_url}/door/v2/readback/{urllib.parse.quote(job_id, safe='')}",
                "job_bound_readback_url": f"{base_url}/door/v2/readback/{urllib.parse.quote(job_id, safe='')}",
    ...
    def job_request_id(self, job_id: str) -> str | None:
        job_name = f"{job_id}.childjob.json"
        for path in [
            self.processed / job_name,
            self.dropzone / job_name,
            self.inbox / job_name,
            self.rejected / job_name,
        ]:
            if not path.exists():
                continue
            try:
                ...
                self.log("readback_job_json_read_failed", job_id=job_id, path=str(path), error=str(exc))
                continue
    ...
    def receipt_bound_readback(self, job_id: str) -> dict[str, Any]:
        if not JOB_RE.match(job_id):
            return {
                "verdict": "UNKNOWN / JOB NOT FOUND",
                "job_id": job_id,
                "request_id": None,
                "receipt_path": None,
                "receipt_sha256": None,
                ...
                "state": "UNKNOWN",
                "boundary": BOUNDARY,
                "error": "job_id outside expected pattern",
            }

        receipt_path = self.outbox / f"{job_id}.receipt.txt"
        job_name = f"{job_id}.childjob.json"
        dropzone_path = self.dropzone / job_name
        ...
        request_id = self.job_request_id(job_id)

        if receipt_path.exists():
            verdict = "PASS / RECEIPT FOUND"
            state = "PROCESSED_PASS"
            error = None
        elif rejected_paths:
            verdict = "FAIL / JOB REJECTED"
            state = "REJECTED"
            error = "job rejected by Child Shell"
        elif any(path.exists() for path in [dropzone_path, inbox_path, processed_path]):
            verdict = "WAITING / JOB CREATED BUT RECEIPT NOT FOUND YET"
            state = "WAITING"
            error = None
        else:
            verdict = "UNKNOWN / JOB NOT FOUND"
            state = "UNKNOWN"
            error = "matching job and receipt paths not found"
        ...
        return {
            ...
            "processed_path": str(processed_path) if processed_path.exists() else None,
            "rejected_path": str(rejected_paths[0]) if rejected_paths else None,
            "state": state,
            "boundary": BOUNDARY,
            "error": error,
        }
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802 - BaseHTTPRequestHandler API
        ...
        is_readback = parsed.path.startswith("/door/v2/readback/") and len(parsed.path.split("/")) == 5
        if parsed.path not in ["/door/v2/status", "/door/v2/probe", "/door/v2/probe_async", "/door/v2/receipt"] and not is_print_probe and not is_readback:
        ...
        try:
            ...
            if is_readback:
                readback_id = urllib.parse.unquote(parsed.path.rsplit("/", 1)[-1])
                payload = self.door.receipt_bound_readback(readback_id)
                status_code = 200
                if payload["state"] == "UNKNOWN":
                    status_code = 404
                elif payload["state"] == "REJECTED":
                    status_code = 409
