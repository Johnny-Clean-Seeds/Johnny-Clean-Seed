...
6. The accepted probe response includes `/door/v2/readback/<job_id>` bound to that exact job.
7. Local `/door/v2/readback/<job_id>` checks only matching job/receipt paths and returns the exact receipt SHA256.
8. Local `/door/v2/status` still refreshes from actual OUTBOX/PROCESSED/REJECTED evidence and shows `PROCESSED_PASS` plus receipt SHA256.
9. The used print is burned, next print is promoted to current, and a new next print is generated.
10. The local-only reset/reseed script is proved or dry-run proved.
11. Temporary tunnel status/probe/readback proof works after local proof.
...
`/door/v2/status -> /door/v2/probe/<print> -> /door/v2/readback/<job_id>`

`/door/v2/status` remains the compact general mouth. V2F adds job-bound readback so final proof can verify the exact job created by the exact probe.
...
Readback checks only exact job, receipt, processed, and rejected paths for the supplied job id. It does not use latest status as proof.
