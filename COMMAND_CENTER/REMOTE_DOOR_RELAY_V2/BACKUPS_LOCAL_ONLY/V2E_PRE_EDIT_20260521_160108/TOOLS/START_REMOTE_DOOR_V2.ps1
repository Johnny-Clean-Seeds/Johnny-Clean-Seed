...
$TokenResult = $null
if (-not $ReuseToken -or -not (Test-TokenStateActive -Path $TokenStatePath)) {
...
$Summary = [ordered]@{
    ...
    port = $ChosenPort
    pid = $Process.Id
    token = if ($TokenResult) { $TokenResult.token } else { $null }
    token_note = if ($TokenResult) { "New token printed once; not stored in plaintext." } else { "Reused existing token; plaintext not available from disk." }
    ...
    status_url_template = "http://127.0.0.1:$ChosenPort/door/v2/status?token=TOKEN"
    probe_async_url_template = "http://127.0.0.1:$ChosenPort/door/v2/probe_async?token=TOKEN&nonce=NONCE"
    latest_proof_flow = "/door/v2/status -> /door/v2/probe_async -> /door/v2/status"
