...
Write-Host "=== ROOT HELD GROUP ROUTE OR HOLD DECISION READ ONLY REVIEW ==="
...
if ($PrepRows.Count -ne 42) {
    Add-Blocker "PREP_ROW_COUNT_MISMATCH: expected 42 parsed rows from prep card :: got $($PrepRows.Count)"
}
...
if ($MissingCount -gt 0) {
    Add-Blocker "PREP_ROWS_MISSING_AT_REVIEW_TIME: $MissingCount"
}
if ($HashChangedCount -gt 0) {
    Add-Blocker "PREP_ROWS_HASH_CHANGED_AT_REVIEW_TIME: $HashChangedCount"
}
...
if ($ExtraRootNonScriptCount -gt 0) {
    Add-Blocker "EXTRA_NON_SCRIPT_ROOT_FILES_AFTER_PREP_SNAPSHOT: $ExtraRootNonScriptCount"
}
...
Write-Host "=== ROOT HELD GROUP ROUTE OR HOLD DECISION READ ONLY REVIEW COMPLETE ==="
