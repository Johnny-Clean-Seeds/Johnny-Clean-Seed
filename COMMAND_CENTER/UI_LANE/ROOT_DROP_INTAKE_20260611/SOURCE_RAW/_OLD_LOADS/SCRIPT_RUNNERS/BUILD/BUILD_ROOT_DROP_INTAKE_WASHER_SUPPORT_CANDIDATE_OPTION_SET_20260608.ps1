...
function Write-BlockerAndExit {
    ...
    $BlockerPath = Join-Path $Lane "BLOCKER__ROOT_DROP_INTAKE_WASHER_SUPPORT_CANDIDATE_OPTION_SET_20260608.md"
    $Timestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"

    $BlockerText = @"
# BLOCKER__ROOT_DROP_INTAKE_WASHER_SUPPORT_CANDIDATE_OPTION_SET_20260608
...
Final verdict:
BLOCKER_ROOT_DROP_INTAKE_WASHER_SUPPORT_CANDIDATE_OPTION_SET_NOT_COMPLETE

DoesNotProve:
This blocker file does not prove the support candidate review failed. It proves this bounded support-candidate option-set runner stopped before valid completion.
"@
    ...
    ""
    "=== ROOT DROP INTAKE WASHER SUPPORT CANDIDATE OPTION SET BLOCKED ==="
    "blocker_path: $BlockerPath"
    "blocker_sha256: $BlockerHash"
    "reason: $Reason"
    "detail: $Detail"
    "final_verdict: BLOCKER_ROOT_DROP_INTAKE_WASHER_SUPPORT_CANDIDATE_OPTION_SET_NOT_COMPLETE"
    exit 1
}
...
"=== ROOT DROP INTAKE WASHER SUPPORT CANDIDATE OPTION SET ==="
...
$ClassCountLines = foreach ($g in $ClassGroups) {
    "- $($g.Name): $($g.Count)"
}

$GuardrailLines = foreach ($item in ($Items | Where-Object { $_.Option -eq "OPTION_KEEP_AS_SUPPORT_GUARDRAIL_CANDIDATE" })) {
    "- $($item.ObservedPath)"
}

$PendingLines = foreach ($item in ($Items | Where-Object { $_.Option -eq "OPTION_KEEP_AS_SUPPORT_CANDIDATE_PENDING_REVIEW" })) {
    "- $($item.ObservedPath)"
}
...
Status: SUPPORT_CANDIDATE_OPTION_SET / READ_ONLY / NO_PROMOTION / NO_MOVES / NO_ROUTING / NO_GIT / NOT_DOCTRINE
...
Purpose:
Condense the 2 support candidate review cards into a small option set.
...
ROUGH_LOCAL_IMPORT_FOR_ROOT_DROP_INTAKE_WASHER_SUPPORT_CANDIDATE_OPTION_SET_20260608

Purpose:
Carry the support candidate option set into Git as rough_local hash truth only.
...
final_verdict:
ROOT_DROP_INTAKE_WASHER_SUPPORT_CANDIDATE_OPTION_SET_READY_WITH_SCOPE_LIMIT_NOTE
"@
...
$ReceiptText = @"
ROOT_DROP_INTAKE_WASHER_SUPPORT_CANDIDATE_OPTION_SET_RECEIPT_20260608
Created: $Timestamp
...
next_build_chunk_selected: ROUGH_LOCAL_IMPORT_FOR_ROOT_DROP_INTAKE_WASHER_SUPPORT_CANDIDATE_OPTION_SET_20260608
final_verdict: ROOT_DROP_INTAKE_WASHER_SUPPORT_CANDIDATE_OPTION_SET_READY_WITH_SCOPE_LIMIT_NOTE
"@
...
"=== ROOT DROP INTAKE WASHER SUPPORT CANDIDATE OPTION SET COMPLETE ==="
"option_set_path: $OutputPath"
