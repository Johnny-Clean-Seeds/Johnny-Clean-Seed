...
Write-Host "XxXxX ===== POWERSHELL DELIVERY LINT RECOVERY START ===== XxXxX"
...
$CurrentDirty = @(Get-GitStatusPaths)
$Unexpected = @($CurrentDirty | Where-Object { $ExpectedDirty -notcontains $_ })
$MissingExpected = @($ExpectedDirty | Where-Object { $CurrentDirty -notcontains $_ })
...
Recovery allowed only the expected dirty paths from the stopped run, then completed anchor/status/receipt/save.
