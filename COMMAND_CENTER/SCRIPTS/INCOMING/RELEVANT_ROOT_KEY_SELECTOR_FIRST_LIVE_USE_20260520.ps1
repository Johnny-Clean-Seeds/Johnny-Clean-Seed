...
Write-Host "XxXxX ===== RELEVANT ROOT KEY SELECTOR FIRST LIVE USE SAVE START ===== XxXxX"
...
foreach ($Target in @($LiveUsePath, $InstantiationPath, $ReceiptPath)) {
...
$LiveUseLines = @(
    ...
    "## Control State",
    "",
    "Current clean point before this save: " + $HeadBefore,
    "",
...
$InstantiationLines = @(
    ...
    "## Status",
    "",
    "CODE CABINET INSTANTIATION / FIRST LIVE USE SAVE.",
    "",
    "## Trigger",
    "",
    "The next nontrivial Mr.Kleen move after selector save was a TODO triage read.",
    "",
    "## Task",
    "",
...
Write-NewCleanFile -Path $LiveUsePath -Text (Join-TextLines -Lines $LiveUseLines)
...
Require-Text -Path $InstantiationPath -Needle "FIRST LIVE USE SAVE"
...
Require-Text -Path $ReceiptPath -Needle "PASS WITH GUARDRAILS / FIRST LIVE USE."
...
Write-Host "RELEVANT ROOT KEY SELECTOR FIRST LIVE USE SAVED"
...
Write-Host "Verdict: PASS WITH GUARDRAILS / FIRST LIVE USE"
