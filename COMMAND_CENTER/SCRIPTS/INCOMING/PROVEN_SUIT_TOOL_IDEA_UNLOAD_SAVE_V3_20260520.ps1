...
function Append-Once {
    ...
    if ($null -eq $Lines) {
        ...
        $TextToAppend = (($Lines | ForEach-Object { [string]$_ }) -join [Environment]::NewLine)
    }
...
Write-Host 'XxXxX ===== PROVEN SUIT TOOL IDEA UNLOAD SAVE V3 START ===== XxXxX'
...
$InstantiationLines = @(
    ...
    '',
    'V1 stopped at parser-check before execution because dynamic PowerShell text construction produced parser errors.',
    '',
    'V2 stopped before writing because mandatory string-array binding rejected blank lines during line joining.',
    '',
    '## Task',
    '',
