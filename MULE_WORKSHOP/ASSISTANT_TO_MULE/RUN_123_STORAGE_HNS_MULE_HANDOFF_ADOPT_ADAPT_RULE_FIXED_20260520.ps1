& {
    ...
    Write-Host "Repair: This fixed helper avoids markdown backtick code spans inside PowerShell strings."
    Write-Host ""
    ...
    foreach ($Name in $RequiredNames) {
        ...
        if (-not $Found) {
            throw "STOP. Required handoff file not found in 123: $Name. Put the repair packet files in Desktop\123, then rerun this helper."
        }
    ...
    if (-not $ResolvedOptional.ContainsKey("HOUSE_NERVOUS_SYSTEM_ALARM_DEBUGGER_MESH_RESEARCH_PACKET_20260520.md")) {
        ...
        @(
            ...
            'HOUSE_NERVOUS_SYSTEM_ALARM_DEBUGGER_MESH_RESEARCH_PACKET_20260520.md',
            '',
            'That file was not found in Desktop\123 during this handoff placement.',
            '',
    ...
    @(
        ...
        '## Rule',
        '',
        'C:\Users\13527\Desktop\123 is the assistant outside storage unit.',
        '',
    ...
    $AddPaths = @($Rule123, $RuleAdopt, $Failure, $StatusFile, $AnchorFile, $ActiveHandoffRoot)
    if ($ArchivedOld -eq "YES") {
        $AddPaths += $ArchiveRoot
    }
    ...
    $CleanupMessages = @()
    $Helpers = @(Get-ChildItem -LiteralPath $Unit123 -File -Filter "RUN_123_STORAGE_HNS_MULE_HANDOFF*.ps1" -ErrorAction SilentlyContinue)
