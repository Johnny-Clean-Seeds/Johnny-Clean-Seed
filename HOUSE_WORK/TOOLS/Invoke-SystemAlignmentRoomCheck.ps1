...
Add-RequiredPath $required 'Assistant Context Carrier Room' (Join-Path $RepoRoot 'HOUSE_WORK\SYSTEM_ALIGNMENT_ROOM\ASSISTANT_CONTEXT_CARRIER_20260531\README.md') 'outside-helper need-to-know support' 'not doctrine'
Add-RequiredPath $required 'Need-To-Know Load Rules' (Join-Path $RepoRoot 'HOUSE_WORK\SYSTEM_ALIGNMENT_ROOM\ASSISTANT_CONTEXT_CARRIER_20260531\NEED_TO_KNOW_LOAD_RULES_20260531.md') 'helper file-scope rule' 'support only'
Add-RequiredPath $required 'Assistant Context Carrier Generator' (Join-Path $RepoRoot 'HOUSE_WORK\TOOLS\Invoke-AssistantContextCarrier.ps1') 'local-only current carrier generator' 'read/report only'
