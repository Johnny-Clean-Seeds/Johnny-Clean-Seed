...
$ParserErrorCapturePath = Join-Path $PacketDir "ROW_001_GIT_ADJUDICATOR_V1_PARSER_ERROR_CAPTURE_$DateKey.md"

$PriorParserErrorText = @'
...
$ParserErrorCapture = $ParserErrorCaptureTemplate.
  Replace('__DATEKEY__', $DateKey).
  Replace('__RUNID__', $RunId).
  Replace('__PRIOR_PARSER_ERROR__', $PriorParserErrorText).
  Replace('__PRIOR_V12_RUNTIME_ERROR__', $PriorV12RuntimeErrorText)

Write-Utf8NoBom $ParserErrorCapturePath $ParserErrorCapture
...
foreach ($cmd in $commandAsts) {
  ...
  if ($name -eq "git") {
    if ($text -eq "& git @GitArgs 2>`$null" -or $text -eq "& git @GitArgs 2>$null") {
      ...
      $failures.Add(("Unknown direct git invocation at line {0}: {1}" -f $line, $text)) | Out-Null
    }
  }
  ...
  if ($name -eq "Read-GitValue") {
    if ($text -match 'Read-GitValue\s+-WorkDir\s+\$RepoRoot\s+-GitArgs\s+@\("rev-parse",\s*"HEAD"\)\s+-Fallback') {
      ...
      $failures.Add(("Unknown Read-GitValue caller at line {0}: {1}" -f $line, $text)) | Out-Null
    }
  }
}
...
foreach ($m in [regex]::Matches($TargetText, $mutationPattern, [System.Text.RegularExpressions.RegexOptions]::IgnoreCase)) {
  ...
  $failures.Add(("Mutation-shaped git text found at line {0}: {1}" -f $line, $m.Value)) | Out-Null
}
...
ParserErrorCapture: $ParserErrorCapturePath
ParserErrorCaptureSha256: $(Get-Sha256 $ParserErrorCapturePath)
Table: $TablePath
...
Write-Host "ParserErrorCapture: $ParserErrorCapturePath"
Write-Host "ParserErrorCaptureSha256: $(Get-Sha256 $ParserErrorCapturePath)"
