<#
SCRIPT NAME:
COMMAND_CENTER_UI_LANE_SAVE_AND_COMMIT_GATE_V1_3_20260606.ps1

PURPOSE:
Repair and rerun the repo-synced Command Center UI lane save/commit gate.

WHY V1.3 EXISTS:
V1.2 fixed native git capture, but authorized mode failed when git add received
305 path arguments directly:
"The filename or extension is too long."

V1.3 uses Git's pathspec-from-file mode:
git add --pathspec-from-file=<manifest>
This keeps the command line short while still staging only manifest-approved files.
...
$RunRoot = Join-Path $SaveCommitRoot ("COMMAND_CENTER_UI_LANE_SAVE_AND_COMMIT_GATE_V1_3_" + $RunStamp)

$CurrentSaveCommitStatusMd = Join-Path $SaveCommitRoot "CURRENT_COMMAND_CENTER_UI_LANE_SAVE_AND_COMMIT_STATUS_V1_3.md"
$CurrentSaveCommitStatusJson = Join-Path $SaveCommitRoot "CURRENT_COMMAND_CENTER_UI_LANE_SAVE_AND_COMMIT_STATUS_V1_3.json"

$SaveCommitPacket = Join-Path $RunRoot "COMMAND_CENTER_UI_LANE_SAVE_AND_COMMIT_PACKET_V1_3_20260606.md"
$ApprovedSyncedFiles = Join-Path $RunRoot "APPROVED_SYNCED_FILES__COMMAND_CENTER_UI_LANE_SAVE_AND_COMMIT_V1_3_20260606.txt"
$GitAddPathspecFile = Join-Path $RunRoot "GIT_ADD_PATHSPEC_FROM_FILE__COMMAND_CENTER_UI_LANE_V1_3_20260606.txt"
$GitStatusBeforeFile = Join-Path $RunRoot "GIT_STATUS_BEFORE__COMMAND_CENTER_UI_LANE_SAVE_AND_COMMIT_V1_3_20260606.txt"
$GitStatusAfterFile = Join-Path $RunRoot "GIT_STATUS_AFTER__COMMAND_CENTER_UI_LANE_SAVE_AND_COMMIT_V1_3_20260606.txt"
$PreExistingStagedFile = Join-Path $RunRoot "PRE_EXISTING_STAGED_FILES__COMMAND_CENTER_UI_LANE_SAVE_AND_COMMIT_V1_3_20260606.txt"
$StagedFilesFile = Join-Path $RunRoot "STAGED_FILES__COMMAND_CENTER_UI_LANE_SAVE_AND_COMMIT_V1_3_20260606.txt"
$CommitOutputFile = Join-Path $RunRoot "GIT_COMMIT_OUTPUT__COMMAND_CENTER_UI_LANE_SAVE_AND_COMMIT_V1_3_20260606.txt"
$NextObjectCard = Join-Path $RunRoot "NEXT_OBJECT_CARD__COMMAND_CENTER_UI_LANE_PUSH_GATE_V1_20260606.md"
$PendingCard = Join-Path $RunRoot "PENDING_CARD__COMMAND_CENTER_UI_LANE_SAVE_AND_COMMIT_V1_3_20260606.md"
$Receipt = Join-Path $RunRoot "RECEIPT__COMMAND_CENTER_UI_LANE_SAVE_AND_COMMIT_GATE_V1_3_20260606.md"
$ErrorLedger = Join-Path $RunRoot "ERROR_LEDGER__COMMAND_CENTER_UI_LANE_SAVE_AND_COMMIT_GATE_V1_3_20260606.md"
$EntryOutput = Join-Path $RunRoot "OUTPUT__WORK_ENTRYPOINT_VERIFY_FOR_SAVE_AND_COMMIT_GATE_V1_3.txt"
...
$saveCommitStatus = "SAVE_AND_COMMIT_V1_3_PENDING_AUTHORIZATION"
...
try {
    ...
    elseif (-not $NoEntryRun) {
        & $WorkEntryScript -WorkIntent "COMMAND_CENTER_UI_LANE_SAVE_AND_COMMIT_GATE_V1_3" -SelectedLane "COMMAND_CENTER_UI_LANE" *>&1 |
            Tee-Object -FilePath $EntryOutput | Out-Host
        $actions += ("RAN_WORK_ENTRYPOINT_FOR_SAVE_AND_COMMIT_GATE_V1_3: " + $WorkEntryScript)
    }
    else {
    ...
    if (-not (Test-Path -LiteralPath $RepoSyncStatusMd)) {
        Add-SaveCommitError -Category "REPO_SYNC_STATUS_MISSING" -Phase "READ_REPO_SYNC_STATUS" -Message $RepoSyncStatusMd -Resolution "Run repo sync pre-commit gate before save/commit V1.3."
    }
    else {
    ...
    if ($syncedFilesManifest -eq "UNKNOWN" -or -not (Test-Path -LiteralPath $syncedFilesManifest)) {
        ...
        $manifestPaths | Set-Content -LiteralPath $GitAddPathspecFile -Encoding UTF8
        $actions += ("READ_SYNCED_FILES_MANIFEST: " + $syncedFilesManifest)
        $actions += ("WROTE_GIT_ADD_PATHSPEC_FROM_FILE: " + $GitAddPathspecFile)
    }
    ...
    if ($AuthorizeCommit -and $commitAuthorized -eq "true" -and @($errors).Count -eq 0) {
        $addResult = Invoke-GitCapture -ArgList @("add", "--pathspec-from-file=$GitAddPathspecFile")
        if ($addResult.ExitCode -ne 0) {
            Add-SaveCommitError -Category "GIT_ADD_PATHSPEC_FROM_FILE_FAILED" -Phase "GIT_ADD" -Message $addResult.Text -Resolution "Review Git pathspec-from-file support and manifest paths."
        }
        else {
            $actions += "GIT_ADD_PATHSPEC_FROM_FILE_COMPLETE"
        }
        ...
        if (@($errors).Count -eq 0) {
            if (@($stagedAfterAdd).Count -lt 1) {
                $saveCommitStatus = "SAVE_AND_COMMIT_V1_3_NO_CHANGES_TO_COMMIT"
                ...
                if ($commitResult.ExitCode -ne 0) {
                    ...
                    $committedHere = "true"
                    $saveCommitStatus = "SAVE_AND_COMMIT_V1_3_COMPLETE"
    ...
    Add-SaveCommitError -Category "SAVE_AND_COMMIT_V1_3_GATE_EXCEPTION" -Phase "TOP_LEVEL" -Message $_.Exception.Message -Resolution "Review save/commit V1.3 error ledger."
    $saveCommitStatus = "SAVE_AND_COMMIT_V1_3_GATE_EXCEPTION"
    $nextLegalObject = "FIX_SAVE_AND_COMMIT_V1_3_GATE_EXCEPTION"
}

$errorLines = @()
$errorLines += "# ERROR LEDGER"
$errorLines += "## COMMAND CENTER UI LANE SAVE AND COMMIT GATE V1.3"
$errorLines += ""
...
$pendingLines += "## COMMAND CENTER UI LANE SAVE AND COMMIT V1.3"
$pendingLines += ""
...
$packetLines += "# Repair"
$packetLines += ""
$packetLines += "RepairFrom: COMMAND_CENTER_UI_LANE_SAVE_AND_COMMIT_GATE_V1_2"
$packetLines += "RepairReason: git add with hundreds of direct path arguments exceeded Windows command-line/process start boundary."
$packetLines += "RepairMethod: git add --pathspec-from-file with a short argument list."
$packetLines += ""
$packetLines += "# Files"
$packetLines += ""
$packetLines += ("ApprovedSyncedFiles: " + $ApprovedSyncedFiles)
$packetLines += ("GitAddPathspecFile: " + $GitAddPathspecFile)
...
$nextLines += ""
if ($saveCommitStatus -eq "SAVE_AND_COMMIT_V1_3_COMPLETE") {
    $nextLines += "A later push gate may be prepared if push is explicitly authorized."
}
elseif ($saveCommitStatus -eq "SAVE_AND_COMMIT_V1_3_NO_CHANGES_TO_COMMIT") {
...
$statusObj = [pscustomobject]@{
    ...
    GitAddPathspecFile = $GitAddPathspecFile
...
$statusLines = @()
$statusLines += "# CURRENT COMMAND CENTER UI LANE SAVE AND COMMIT STATUS V1.3"
$statusLines += ""
...
$statusLines += ("GitAddPathspecFile: " + $GitAddPathspecFile)
...
$receiptLines += "## COMMAND CENTER UI LANE SAVE AND COMMIT GATE V1.3"
$receiptLines += ""
...
Write-Host ""
Write-Host "Command Center UI lane save and commit gate V1.3 complete."
...
Write-Host ""
Write-Host "DONE_MARKER: COMMAND_CENTER_UI_LANE_SAVE_AND_COMMIT_GATE_V1_3_FINALIZED"
