<#
SCRIPT NAME:
COMMAND_CENTER_UI_LANE_SAVE_AND_COMMIT_GATE_V1_2_20260606.ps1

PURPOSE:
Repair and rerun the repo-synced Command Center UI lane save/commit gate.

WHY V1.2 EXISTS:
V1.1 failed before staging because PowerShell/native git capture raised:
"StandardOutputEncoding is only supported when standard output is redirected."

V1.2 replaces native command capture with System.Diagnostics.ProcessStartInfo,
with stdout/stderr explicitly redirected. It stages only the repo-sync manifest files.
...
$RunRoot = Join-Path $SaveCommitRoot ("COMMAND_CENTER_UI_LANE_SAVE_AND_COMMIT_GATE_V1_2_" + $RunStamp)

$CurrentSaveCommitStatusMd = Join-Path $SaveCommitRoot "CURRENT_COMMAND_CENTER_UI_LANE_SAVE_AND_COMMIT_STATUS_V1_2.md"
$CurrentSaveCommitStatusJson = Join-Path $SaveCommitRoot "CURRENT_COMMAND_CENTER_UI_LANE_SAVE_AND_COMMIT_STATUS_V1_2.json"

$SaveCommitPacket = Join-Path $RunRoot "COMMAND_CENTER_UI_LANE_SAVE_AND_COMMIT_PACKET_V1_2_20260606.md"
$ApprovedSyncedFiles = Join-Path $RunRoot "APPROVED_SYNCED_FILES__COMMAND_CENTER_UI_LANE_SAVE_AND_COMMIT_V1_2_20260606.txt"
$GitStatusBeforeFile = Join-Path $RunRoot "GIT_STATUS_BEFORE__COMMAND_CENTER_UI_LANE_SAVE_AND_COMMIT_V1_2_20260606.txt"
$GitStatusAfterFile = Join-Path $RunRoot "GIT_STATUS_AFTER__COMMAND_CENTER_UI_LANE_SAVE_AND_COMMIT_V1_2_20260606.txt"
$PreExistingStagedFile = Join-Path $RunRoot "PRE_EXISTING_STAGED_FILES__COMMAND_CENTER_UI_LANE_SAVE_AND_COMMIT_V1_2_20260606.txt"
$StagedFilesFile = Join-Path $RunRoot "STAGED_FILES__COMMAND_CENTER_UI_LANE_SAVE_AND_COMMIT_V1_2_20260606.txt"
$CommitOutputFile = Join-Path $RunRoot "GIT_COMMIT_OUTPUT__COMMAND_CENTER_UI_LANE_SAVE_AND_COMMIT_V1_2_20260606.txt"
$NextObjectCard = Join-Path $RunRoot "NEXT_OBJECT_CARD__COMMAND_CENTER_UI_LANE_PUSH_GATE_V1_20260606.md"
$PendingCard = Join-Path $RunRoot "PENDING_CARD__COMMAND_CENTER_UI_LANE_SAVE_AND_COMMIT_V1_2_20260606.md"
$Receipt = Join-Path $RunRoot "RECEIPT__COMMAND_CENTER_UI_LANE_SAVE_AND_COMMIT_GATE_V1_2_20260606.md"
$ErrorLedger = Join-Path $RunRoot "ERROR_LEDGER__COMMAND_CENTER_UI_LANE_SAVE_AND_COMMIT_GATE_V1_2_20260606.md"
$EntryOutput = Join-Path $RunRoot "OUTPUT__WORK_ENTRYPOINT_VERIFY_FOR_SAVE_AND_COMMIT_GATE_V1_2.txt"
...
try {
    ...
    elseif (-not $NoEntryRun) {
        & $WorkEntryScript -WorkIntent "COMMAND_CENTER_UI_LANE_SAVE_AND_COMMIT_GATE_V1_2" -SelectedLane "COMMAND_CENTER_UI_LANE" *>&1 |
            Tee-Object -FilePath $EntryOutput | Out-Host
        $actions += ("RAN_WORK_ENTRYPOINT_FOR_SAVE_AND_COMMIT_GATE_V1_2: " + $WorkEntryScript)
    }
    else {
    ...
    if (-not (Test-Path -LiteralPath $RepoSyncStatusMd)) {
        Add-SaveCommitError -Category "REPO_SYNC_STATUS_MISSING" -Phase "READ_REPO_SYNC_STATUS" -Message $RepoSyncStatusMd -Resolution "Run repo sync pre-commit gate before save/commit V1.2."
    }
    else {
        ...
        if ($repoSyncNextLegalObject -ne "COMMAND_CENTER_UI_LANE_SAVE_AND_COMMIT_GATE_V1_1" -and $repoSyncNextLegalObject -ne "COMMAND_CENTER_UI_LANE_SAVE_AND_COMMIT_GATE_V1_2") {
    ...
    if ($AuthorizeCommit -and $commitAuthorized -eq "true" -and @($errors).Count -eq 0) {
        ...
        if (@($errors).Count -eq 0) {
            if (@($stagedAfterAdd).Count -lt 1) {
                $saveCommitStatus = "SAVE_AND_COMMIT_V1_2_NO_CHANGES_TO_COMMIT"
                ...
                if ($commitResult.ExitCode -ne 0) {
                    ...
                    $saveCommitStatus = "SAVE_AND_COMMIT_V1_2_BLOCKED"
                    $nextLegalObject = "FIX_SAVE_AND_COMMIT_V1_2_BLOCKERS"
                }
                else {
                    $committedHere = "true"
                    $saveCommitStatus = "SAVE_AND_COMMIT_V1_2_COMPLETE"
    ...
    Add-SaveCommitError -Category "SAVE_AND_COMMIT_V1_2_GATE_EXCEPTION" -Phase "TOP_LEVEL" -Message $_.Exception.Message -Resolution "Review save/commit V1.2 error ledger."
    $saveCommitStatus = "SAVE_AND_COMMIT_V1_2_GATE_EXCEPTION"
    $nextLegalObject = "FIX_SAVE_AND_COMMIT_V1_2_GATE_EXCEPTION"
}

$errorLines = @()
$errorLines += "# ERROR LEDGER"
$errorLines += "## COMMAND CENTER UI LANE SAVE AND COMMIT GATE V1.2"
$errorLines += ""
...
$pendingLines += "## COMMAND CENTER UI LANE SAVE AND COMMIT V1.2"
$pendingLines += ""
...
$packetLines += "# Repair"
$packetLines += ""
$packetLines += "RepairFrom: COMMAND_CENTER_UI_LANE_SAVE_AND_COMMIT_GATE_V1_1"
$packetLines += "RepairReason: Native git capture failed with StandardOutputEncoding error."
$packetLines += "RepairMethod: System.Diagnostics.ProcessStartInfo with stdout/stderr redirected."
$packetLines += ""
$packetLines += "# Files"
$packetLines += ""
...
$nextLines += ""
if ($saveCommitStatus -eq "SAVE_AND_COMMIT_V1_2_COMPLETE") {
    $nextLines += "A later push gate may be prepared if push is explicitly authorized."
}
elseif ($saveCommitStatus -eq "SAVE_AND_COMMIT_V1_2_NO_CHANGES_TO_COMMIT") {
...
$statusLines = @()
$statusLines += "# CURRENT COMMAND CENTER UI LANE SAVE AND COMMIT STATUS V1.2"
$statusLines += ""
...
$receiptLines += "## COMMAND CENTER UI LANE SAVE AND COMMIT GATE V1.2"
$receiptLines += ""
...
Write-Host ""
Write-Host "Command Center UI lane save and commit gate V1.2 complete."
...
Write-Host ""
Write-Host "DONE_MARKER: COMMAND_CENTER_UI_LANE_SAVE_AND_COMMIT_GATE_V1_2_FINALIZED"
