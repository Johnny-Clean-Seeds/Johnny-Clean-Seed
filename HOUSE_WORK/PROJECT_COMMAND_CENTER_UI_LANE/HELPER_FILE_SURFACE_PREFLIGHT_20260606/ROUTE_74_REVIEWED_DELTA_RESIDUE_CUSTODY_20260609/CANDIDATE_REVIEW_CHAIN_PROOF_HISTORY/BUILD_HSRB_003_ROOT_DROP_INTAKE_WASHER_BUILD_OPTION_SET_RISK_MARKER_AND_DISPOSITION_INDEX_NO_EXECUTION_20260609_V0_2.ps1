...
$OutIndexCsvPath = Join-Path $Lane 'HSRB_003_ROOT_DROP_INTAKE_WASHER_BUILD_OPTION_SET_RISK_MARKER_AND_DISPOSITION_INDEX_NO_EXECUTION_V0_2_20260609.csv'
$OutIndexMdPath = Join-Path $Lane 'HSRB_003_ROOT_DROP_INTAKE_WASHER_BUILD_OPTION_SET_RISK_MARKER_AND_DISPOSITION_INDEX_NO_EXECUTION_V0_2_20260609.md'
$OutIndexPrintPath = Join-Path $Lane 'HSRB_003_ROOT_DROP_INTAKE_WASHER_BUILD_OPTION_SET_RISK_MARKER_AND_DISPOSITION_INDEX_NO_EXECUTION_COPY_PRINT_V0_2_20260609.txt'
$OutReceiptPath = Join-Path $Lane 'HSRB_003_ROOT_DROP_INTAKE_WASHER_BUILD_OPTION_SET_RISK_MARKER_AND_DISPOSITION_INDEX_NO_EXECUTION_RECEIPT_V0_2_20260609.txt'
$V01ErrorFreezePath = Join-Path $Lane 'ERROR_FREEZE__HSRB_003_RISK_MARKER_AND_DISPOSITION_INDEX_V0_1_RISK_CLASS_MISMATCH_AND_BLANK_CSV_SHA_20260609.md'
$FixNotePath = Join-Path $Lane 'FIX_NOTE__HSRB_003_RISK_MARKER_AND_DISPOSITION_INDEX_V0_2_RISK_CLASS_AND_CSV_SHA_REPAIR_20260609.md'
$FixReceiptPath = Join-Path $Lane 'HASH_RECEIPT__HSRB_003_RISK_MARKER_AND_DISPOSITION_INDEX_V0_2_REPAIR_20260609.txt'
...
$v01FreezeLines = @()
$v01FreezeLines += '# Error Freeze - HSRB-003 Risk Marker and Disposition Index V0.1'
$v01FreezeLines += ''
$v01FreezeLines += 'Status: ERROR_FREEZE / GENERATED_HELPER_OUTPUT_DEFECT / NO_EXECUTION / NO_ROUTE / NO_CLEANUP'
$v01FreezeLines += ''
$v01FreezeLines += '## Defect'
$v01FreezeLines += ''
$v01FreezeLines += 'V0.1 wrote a blocker report because the risk-disposition index expected local risk labels that did not match the already-approved contract-first closeout risk labels.'
$v01FreezeLines += ''
$v01FreezeLines += 'Observed V0.1 symptoms from terminal output:'
$v01FreezeLines += ''
$v01FreezeLines += '- contract_gate_passed: False'
$v01FreezeLines += '- output_index_csv_sha256: blank'
$v01FreezeLines += '- blocker_count: 9'
$v01FreezeLines += '- contains_copy_item_count: 1'
$v01FreezeLines += '- contains_git_command_count: 9'
$v01FreezeLines += '- high_risk_command_marker_row_count: 0'
$v01FreezeLines += '- unclassified_risk_marker_count: 0'
$v01FreezeLines += '- physical_actions: move=0 delete=0 rename=0 route=0 execute=0 commit=0 push=0'
$v01FreezeLines += ''
$v01FreezeLines += '## Interpretation'
$v01FreezeLines += ''
$v01FreezeLines += 'The reviewed files were not executed and no physical route/cleanup happened. The failure is in the derived index helper: it did not reuse the exact contract-first RiskClass vocabulary, so all nine risk-marked review-only rows became blockers.'
$v01FreezeLines += ''
$v01FreezeLines += '## Repair rule'
$v01FreezeLines += ''
$v01FreezeLines += 'V0.2 must reuse the contract-first RiskClass vocabulary exactly and must emit a nonblank CSV SHA when the contract passes.'
Write-LinesNoBom -Path $V01ErrorFreezePath -Lines $v01FreezeLines
$v01FreezeSha = Get-Sha256Safe -Path $V01ErrorFreezePath

$fixNoteLines = @()
$fixNoteLines += '# Fix Note - HSRB-003 Risk Marker and Disposition Index V0.2'
$fixNoteLines += ''
$fixNoteLines += 'Status: FIX_NOTE / SAME_OBJECT_REPAIR / NO_EXECUTION / NO_ROUTE / NO_CLEANUP'
$fixNoteLines += ''
$fixNoteLines += '## Repair'
$fixNoteLines += ''
$fixNoteLines += 'V0.2 repairs V0.1 by aligning the risk-disposition index vocabulary with the contract-first closeout RiskClass vocabulary.'
$fixNoteLines += ''
$fixNoteLines += 'Expected accepted review-only classes:'
$fixNoteLines += ''
$fixNoteLines += '- RISK_MARKED_COPY_AND_GIT_REVIEW_ONLY_NOT_CLEARED'
$fixNoteLines += '- RISK_MARKED_COPY_REVIEW_ONLY_NOT_CLEARED'
$fixNoteLines += '- RISK_MARKED_GIT_REVIEW_ONLY_NOT_CLEARED'
$fixNoteLines += '- NO_COMMAND_RISK_MARKER_REVIEW_ONLY_NOT_CLEARED'
$fixNoteLines += ''
$fixNoteLines += 'High-risk classes remain blockers:'
$fixNoteLines += ''
$fixNoteLines += '- BLOCKED_HIGH_RISK_STATIC_MARKER_REVIEW_REQUIRED'
$fixNoteLines += ''
$fixNoteLines += 'No helper execution, physical routing, cleanup, commit, push, or doctrine promotion is authorized.'
Write-LinesNoBom -Path $FixNotePath -Lines $fixNoteLines
...
if ($contractGatePassed) {
    ...
    $finalVerdict = 'HSRB_003_RISK_MARKER_AND_DISPOSITION_INDEX_V0_2_WRITTEN_WITH_REVIEW_ONLY_DISPOSITIONS_NO_PHYSICAL_ACTION'
} else {
    $nextSingleAction = 'STOP_AND_REVIEW_HSRB_003_RISK_MARKER_AND_DISPOSITION_INDEX_BLOCKERS_NO_EXECUTION'
    $finalVerdict = 'HSRB_003_RISK_MARKER_AND_DISPOSITION_INDEX_V0_2_WRITTEN_WITH_BLOCKERS_NO_PHYSICAL_ACTION'
}
...
$lines = @()
$lines += '# HSRB-003 Root Drop Intake Washer Build Option Set - Risk Marker and Disposition Index - V0.2'
$lines += ''
...
$receiptLines = @()
$receiptLines += 'HSRB-003 RISK MARKER AND DISPOSITION INDEX RECEIPT V0.2'
...
$fixReceiptLines = @()
$fixReceiptLines += 'HSRB-003 RISK MARKER AND DISPOSITION INDEX V0.2 REPAIR RECEIPT'
$fixReceiptLines += ('created_at_local: {0}' -f (Get-Date -Format 'yyyy-MM-dd HH:mm:ss zzz'))
$fixReceiptLines += ('v0_1_error_freeze_path: {0}' -f $V01ErrorFreezePath)
$fixReceiptLines += ('v0_1_error_freeze_sha256: {0}' -f $v01FreezeSha)
$fixReceiptLines += ('fix_note_path: {0}' -f $FixNotePath)
$fixReceiptLines += ('fix_note_sha256: {0}' -f $fixNoteSha)
$fixReceiptLines += ('output_index_csv_path: {0}' -f $OutIndexCsvPath)
$fixReceiptLines += ('output_index_csv_sha256: {0}' -f $indexCsvSha)
$fixReceiptLines += ('output_index_md_path: {0}' -f $OutIndexMdPath)
$fixReceiptLines += ('output_index_md_sha256: {0}' -f $indexMdSha)
$fixReceiptLines += ('output_receipt_path: {0}' -f $OutReceiptPath)
$fixReceiptLines += ('output_receipt_sha256: {0}' -f $receiptSha)
$fixReceiptLines += ('contract_gate_passed: {0}' -f $contractGatePassed)
$fixReceiptLines += ('blocker_count: {0}' -f $blockerCount)
$fixReceiptLines += ('final_verdict: {0}' -f $finalVerdict)
$fixReceiptLines += ('physical_actions: move={0} delete={1} rename={2} route={3} execute={4} commit={5} push={6}' -f $PhysicalMoves,$PhysicalDeletes,$PhysicalRenames,$PhysicalRoutes,$PhysicalExecutes,$PhysicalCommits,$PhysicalPushes)
Write-LinesNoBom -Path $FixReceiptPath -Lines $fixReceiptLines
$fixReceiptSha = Get-Sha256Safe -Path $FixReceiptPath

'=== HSRB-003 RISK MARKER AND DISPOSITION INDEX V0.2 COMPLETE ==='
