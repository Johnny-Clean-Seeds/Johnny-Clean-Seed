...
$V01ErrorFreezePath = Join-Path $Lane 'ERROR_FREEZE__HSRB_006_NEXT_BATCH_SELECTOR_V0_1_ACTION_NOW_SEMANTIC_BLOCKER_20260609.md'
$FixNotePath = Join-Path $Lane 'FIX_NOTE__HSRB_006_NEXT_BATCH_SELECTOR_V0_2_ACTION_NOW_SELECTOR_NORMALIZATION_AND_HIGH_RISK_REVIEW_ONLY_CLASSIFICATION_20260609.md'
$FixReceiptPath = Join-Path $Lane 'HASH_RECEIPT__HSRB_006_NEXT_BATCH_SELECTOR_V0_2_REPAIR_20260609.txt'

$OutSelectedCsv = Join-Path $Lane 'HELPER_SCRIPT_REVIEW_NEXT_BATCH_SELECTOR_HSRB_006_FROM_64_QUEUE_NO_EXECUTION_SELECTED_BATCH_006_V0_2_20260609.csv'
$OutRemainingCsv = Join-Path $Lane 'HELPER_SCRIPT_REVIEW_NEXT_BATCH_SELECTOR_HSRB_006_FROM_64_QUEUE_NO_EXECUTION_REMAINING_AFTER_001_005_V0_2_20260609.csv'
$OutMd = Join-Path $Lane 'HELPER_SCRIPT_REVIEW_NEXT_BATCH_SELECTOR_HSRB_006_FROM_64_QUEUE_NO_EXECUTION_V0_2_20260609.md'
$OutPrint = Join-Path $Lane 'HELPER_SCRIPT_REVIEW_NEXT_BATCH_SELECTOR_HSRB_006_FROM_64_QUEUE_NO_EXECUTION_COPY_PRINT_V0_2_20260609.txt'
$OutReceipt = Join-Path $Lane 'HELPER_SCRIPT_REVIEW_NEXT_BATCH_SELECTOR_HSRB_006_FROM_64_QUEUE_NO_EXECUTION_RECEIPT_V0_2_20260609.txt'
...
foreach ($row in $queueRows) {
    ...
    $sourceActionNow = Get-Cell -Row $row -Names @('ActionNow','Action','ActionStatus')
    ...
    $rowOut = [pscustomobject][ordered]@{
        ...
        ActionNow = [string]$selectorActionNow
...
foreach ($row in $selectedRows) {
    ...
    if (([string]$row.SourceActionNow).Trim().ToUpperInvariant() -ne 'NO') { $sourceActionNowNonNoCount += 1 }
    if (([string]$row.SelectorActionNow).Trim().ToUpperInvariant() -ne 'NO') { $selectorActionNowNonNoCount += 1 }
    ...
    if ($row.HighRiskReviewOnlyMarker) { $highRiskReviewOnlyCount += 1 }
...
if ($selectorActionNowNonNoCount -gt 0) { $blockers += ('SELECTOR_ACTION_NOW_NON_NO_COUNT_{0}' -f $selectorActionNowNonNoCount) }
...
$finalVerdict = if ($contractGatePassed) { 'HELPER_SCRIPT_REVIEW_NEXT_BATCH_SELECTOR_HSRB_006_FROM_64_QUEUE_V0_2_WRITTEN_WITH_SOURCE_ACTION_NOW_PRESERVED_SELECTOR_ACTION_NOW_NO_HIGH_RISK_REVIEW_ONLY_RECURSIVE_DRY_RUN_REQUIRED_NO_PHYSICAL_ACTION' } else { 'HELPER_SCRIPT_REVIEW_NEXT_BATCH_SELECTOR_HSRB_006_FROM_64_QUEUE_V0_2_WRITTEN_WITH_BLOCKERS_NO_PHYSICAL_ACTION' }
...
$fixReceiptLines = @(
    'HASH RECEIPT — HSRB-006 NEXT BATCH SELECTOR V0.2 REPAIR',
...
$mdLines = @()
$mdLines += '# HELPER SCRIPT REVIEW NEXT BATCH SELECTOR — HSRB-006 FROM 64 QUEUE V0.2'
$mdLines += ''
...
$mdLines += "high_risk_review_only_marker_count: $highRiskReviewOnlyCount"
$mdLines += "risk_marked_row_count: $riskMarkedCount"
$mdLines += "unclassified_risk_marker_count: $unclassifiedRiskCount"
$mdLines += "source_action_now_non_no_count: $sourceActionNowNonNoCount"
$mdLines += "selector_action_now_non_no_count: $selectorActionNowNonNoCount"
...
$receiptLines = @()
$receiptLines += 'HASH RECEIPT — HSRB-006 NEXT BATCH SELECTOR V0.2'
...
Write-Host '=== HELPER SCRIPT REVIEW NEXT BATCH SELECTOR HSRB-006 FROM 64 QUEUE V0.2 COMPLETE ==='
...
Write-Host "high_risk_review_only_marker_count: $highRiskReviewOnlyCount"
