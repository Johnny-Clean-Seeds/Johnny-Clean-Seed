...
Add-Line $fix ''
Add-Line $fix 'Repair: V0.2 rebuilds the HSRB-002 index by joining the selected batch CSV back to the static summary by FileName, preserving TicketID from the selected batch source.'
Add-Line $fix ''
Add-Line $fix 'Forward helper-generation rule candidate: any derived queue, index, closeout, or proof ledger must preserve custody keys from its source surface, especially TicketID, FileName, SHA256, source role, decision/disposition, and source path when available.'
Add-Line $fix ''
...
'=== HSRB-002 TEMPLATE RULE AND ATTEMPT INDEX TICKET ID REPAIR V0.1 COMPLETE ==='
