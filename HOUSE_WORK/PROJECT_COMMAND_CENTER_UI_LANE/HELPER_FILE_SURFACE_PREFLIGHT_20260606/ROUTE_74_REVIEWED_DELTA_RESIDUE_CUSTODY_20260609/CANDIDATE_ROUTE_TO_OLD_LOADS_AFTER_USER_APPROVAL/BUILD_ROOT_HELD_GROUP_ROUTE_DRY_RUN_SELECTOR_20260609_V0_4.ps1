<#
ROOT HELD GROUP ROUTE DRY-RUN SELECTOR 20260609 V0.4
Status: DRY_RUN_SELECTOR_BUILDER_REPAIR_V0_4 / READ_ONLY_CLASSIFICATION / NO_PHYSICAL_ROUTING

Repair reason:
  V0.1 failed under Set-StrictMode because a collection-like value collapsed to a scalar and .Count was called.
  V0.2 repaired scalar count checks but failed on strict parameter binding. V0.3 repaired loose input intake but failed by constructing a regex from an unescaped Windows path. V0.4 removes regex path extraction and uses manual root-path scanning.
...
$ReportName = 'ROOT_HELD_GROUP_ROUTE_DRY_RUN_SELECTOR_V0_4_20260609.md'
$ReceiptName = 'ROOT_HELD_GROUP_ROUTE_DRY_RUN_SELECTOR_RECEIPT_V0_4_20260609.txt'
$ErrorFreezeName = 'ERROR_FREEZE__ROOT_HELD_ROUTE_DRY_RUN_SELECTOR_V0_3_UNESCAPED_WINDOWS_PATH_REGEX_20260609.md'
$FixNoteName = 'FIX_NOTE__ROOT_HELD_ROUTE_DRY_RUN_SELECTOR_V0_4_MANUAL_ROOT_PATH_SCAN_20260609.md'
$FixReceiptName = 'HASH_RECEIPT__ROOT_HELD_ROUTE_DRY_RUN_SELECTOR_V0_4_FIX_20260609.txt'
...
function Get-RootPathCandidatesFromLine {
    param(
    ...
    if ([string]::IsNullOrWhiteSpace($RootPath)) { return @($found) }

    $searchFrom = 0
    while ($searchFrom -lt $Line.Length) {
        $idx = $Line.IndexOf($RootPath, $searchFrom, [System.StringComparison]::OrdinalIgnoreCase)
        if ($idx -lt 0) { break }

        $end = $idx
        while ($end -lt $Line.Length) {
            $ch = $Line[$end]
            if ([char]::IsWhiteSpace($ch) -or $ch -eq '|' -or $ch -eq [char]0x60 -or $ch -eq ']' -or $ch -eq ')' -or $ch -eq ',' -or $ch -eq '"') {
                break
            }
            $end++
        }

        $candidate = $Line.Substring($idx, $end - $idx).Trim()
        if (-not [string]::IsNullOrWhiteSpace($candidate)) {
            $found.Add($candidate) | Out-Null
        }

        $searchFrom = [Math]::Max($end + 1, $idx + $RootPath.Length)
    }
...
function Extract-ExpectedRootRows {
    ...
    foreach ($inputPath in @($inputPathList)) {
        ...
        foreach ($line in @($lines)) {
            $lineNo++
            $paths = @(Get-RootPathCandidatesFromLine -Line ([string]$line) -RootPath $RootPath)
            ...
            foreach ($pathTextRaw in @($paths)) {
                $pathText = ([string]$pathTextRaw).Trim().Trim('`').Trim()
                try {
...
$verdict = if ((Measure-Items $Blockers) -gt 0) {
    'ROOT_HELD_GROUP_ROUTE_DRY_RUN_SELECTOR_V0_4_WRITTEN_WITH_BLOCKERS_REVIEW_REQUIRED'
} elseif ((Measure-Items $liveDeltaRows) -gt 0) {
    'ROOT_HELD_GROUP_ROUTE_DRY_RUN_SELECTOR_V0_4_WRITTEN_WITH_LIVE_DELTAS_REVIEW_REQUIRED'
} else {
    'ROOT_HELD_GROUP_ROUTE_DRY_RUN_SELECTOR_V0_4_READY_WITH_NO_PHYSICAL_ACTION'
}

$freeze = New-Object System.Collections.Generic.List[string]
$freeze.Add('# ERROR FREEZE — ROOT HELD ROUTE DRY-RUN SELECTOR V0.3 UNESCAPED WINDOWS PATH REGEX 20260609') | Out-Null
$freeze.Add('') | Out-Null
...
$freeze.Add('') | Out-Null
$freeze.Add('V0.3 run failed at Extract-ExpectedRootRows with: Invalid pattern `C:\Users\13527\Desktop\123\...`; unescaped Windows backslashes were interpreted as regex escapes such as `\U`.') | Out-Null
$freeze.Add('') | Out-Null
$freeze.Add('## Failure Family') | Out-Null
$freeze.Add('') | Out-Null
$freeze.Add('The parser built a regex from a literal Windows path without escaping backslashes. That is a generated-script parser defect, not a route-plan, Git, or user defect.') | Out-Null
$freeze.Add('') | Out-Null
$freeze.Add('## DoesNotProve') | Out-Null
$freeze.Add('') | Out-Null
$freeze.Add('This error does not prove route failure, Git failure, user misuse, file safety, cleanup approval, or movement approval. It proves only a generated-script path-parser defect.') | Out-Null

$fix = New-Object System.Collections.Generic.List[string]
$fix.Add('# FIX NOTE — ROOT HELD ROUTE DRY-RUN SELECTOR V0.4 MANUAL ROOT PATH SCAN 20260609') | Out-Null
$fix.Add('') | Out-Null
$fix.Add('Status: FIX_NOTE / SAME_FAILURE_FAMILY_REPAIR / NO_PHYSICAL_ROUTING') | Out-Null
$fix.Add('') | Out-Null
$fix.Add('## Fix') | Out-Null
$fix.Add('') | Out-Null
$fix.Add('V0.4 keeps the V0.2 count-normalization repair and V0.3 loose input normalization, then removes regex-based Windows path extraction from `Extract-ExpectedRootRows`. It scans each proof line for the literal project root and manually stops at Markdown/path delimiters. This prevents unescaped `C:\Users...` from becoming an invalid regex pattern.') | Out-Null
$fix.Add('') | Out-Null
...
$report.Add('# ROOT HELD GROUP ROUTE DRY-RUN SELECTOR V0.4 20260609') | Out-Null
$report.Add('') | Out-Null
...
$receipt = @(
    'ROOT HELD GROUP ROUTE DRY-RUN SELECTOR RECEIPT V0.4 20260609',
...
$fixReceipt = @(
    'HASH RECEIPT ROOT HELD ROUTE DRY-RUN SELECTOR V0.4 FIX 20260609',
    "created_at: $(Get-Date -Format o)",
    "error_freeze_path: $ErrorFreezePath",
    "error_freeze_sha256: $ErrorFreezeHash",
    "fix_note_path: $FixNotePath",
...
Write-Host '=== ROOT HELD GROUP ROUTE DRY-RUN SELECTOR V0.4 COMPLETE ==='
