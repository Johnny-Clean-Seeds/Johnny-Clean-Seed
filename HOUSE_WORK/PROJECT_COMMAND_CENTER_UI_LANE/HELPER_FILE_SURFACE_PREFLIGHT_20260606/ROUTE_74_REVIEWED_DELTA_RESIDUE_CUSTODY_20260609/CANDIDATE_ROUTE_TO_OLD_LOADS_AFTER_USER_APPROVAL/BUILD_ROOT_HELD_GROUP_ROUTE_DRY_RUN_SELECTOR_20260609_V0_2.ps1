<#
ROOT HELD GROUP ROUTE DRY-RUN SELECTOR 20260609 V0.2
Status: DRY_RUN_SELECTOR_BUILDER_REPAIR / READ_ONLY_CLASSIFICATION / NO_PHYSICAL_ROUTING

Repair reason:
  V0.1 failed under Set-StrictMode because a collection-like value collapsed to a scalar and .Count was called.
  V0.2 uses Measure-Items array normalization for all count checks and writes an error-freeze/fix-note pair.
...
$ReportName = 'ROOT_HELD_GROUP_ROUTE_DRY_RUN_SELECTOR_V0_2_20260609.md'
$ReceiptName = 'ROOT_HELD_GROUP_ROUTE_DRY_RUN_SELECTOR_RECEIPT_V0_2_20260609.txt'
$ErrorFreezeName = 'ERROR_FREEZE__ROOT_HELD_ROUTE_DRY_RUN_SELECTOR_SCALAR_COUNT_STRICTMODE_20260609.md'
$FixNoteName = 'FIX_NOTE__ROOT_HELD_ROUTE_DRY_RUN_SELECTOR_V0_2_ARRAY_WRAP_20260609.md'
$FixReceiptName = 'HASH_RECEIPT__ROOT_HELD_ROUTE_DRY_RUN_SELECTOR_V0_2_FIX_20260609.txt'
...
function Extract-ExpectedRootRows {
    param(
        [Parameter(Mandatory=$true)][object[]]$InputFilePaths,
...
$routeProofCandidates = @($routeProofList | Sort-Object -Unique)
...
$verdict = if ((Measure-Items $Blockers) -gt 0) {
    'ROOT_HELD_GROUP_ROUTE_DRY_RUN_SELECTOR_V0_2_WRITTEN_WITH_BLOCKERS_REVIEW_REQUIRED'
} elseif ((Measure-Items $liveDeltaRows) -gt 0) {
    'ROOT_HELD_GROUP_ROUTE_DRY_RUN_SELECTOR_V0_2_WRITTEN_WITH_LIVE_DELTAS_REVIEW_REQUIRED'
} else {
    'ROOT_HELD_GROUP_ROUTE_DRY_RUN_SELECTOR_V0_2_READY_WITH_NO_PHYSICAL_ACTION'
}

$freeze = New-Object System.Collections.Generic.List[string]
$freeze.Add('# ERROR FREEZE — ROOT HELD ROUTE DRY-RUN SELECTOR SCALAR COUNT STRICTMODE 20260609') | Out-Null
$freeze.Add('') | Out-Null
...
$freeze.Add('') | Out-Null
$freeze.Add('V0.1 run failed with: The property Count cannot be found on this object. Verify that the property exists.') | Out-Null
$freeze.Add('') | Out-Null
$freeze.Add('## Failure Family') | Out-Null
$freeze.Add('') | Out-Null
$freeze.Add('PowerShell scalar-collapse under Set-StrictMode where a collection-like value may become a scalar and `.Count` is called directly.') | Out-Null
$freeze.Add('') | Out-Null
$freeze.Add('## DoesNotProve') | Out-Null
$freeze.Add('') | Out-Null
$freeze.Add('This error does not prove route failure, Git failure, user misuse, file safety, cleanup approval, or movement approval. It proves only a generated-script count-normalization defect.') | Out-Null

$fix = New-Object System.Collections.Generic.List[string]
$fix.Add('# FIX NOTE — ROOT HELD ROUTE DRY-RUN SELECTOR V0.2 ARRAY WRAP 20260609') | Out-Null
$fix.Add('') | Out-Null
$fix.Add('Status: FIX_NOTE / SAME_FAILURE_FAMILY_REPAIR / NO_PHYSICAL_ROUTING') | Out-Null
$fix.Add('') | Out-Null
$fix.Add('## Fix') | Out-Null
$fix.Add('') | Out-Null
$fix.Add('V0.2 adds `Measure-Items` and normalizes pipeline output through `@(...)` before count checks. Direct scalar-sensitive `.Count` checks were replaced on uncertain values.') | Out-Null
$fix.Add('') | Out-Null
...
$report.Add('# ROOT HELD GROUP ROUTE DRY-RUN SELECTOR V0.2 20260609') | Out-Null
$report.Add('') | Out-Null
...
$report.Add('') | Out-Null
$report.Add('## 2. V0.1 Failure Capture') | Out-Null
$report.Add('') | Out-Null
...
$report.Add('') | Out-Null
$report.Add('This dry-run selector report proves only that the live root was snapshotted, route-plan proof candidates were searched, expected rows were parsed where possible, receptionist tickets were generated, the V0.1 Count failure was frozen, and no physical file action was performed.') | Out-Null
$report.Add('') | Out-Null
...
$receipt = @(
    'ROOT HELD GROUP ROUTE DRY-RUN SELECTOR RECEIPT V0.2 20260609',
...
$fixReceipt = @(
    'HASH RECEIPT ROOT HELD ROUTE DRY-RUN SELECTOR V0.2 FIX 20260609',
    "created_at: $(Get-Date -Format o)",
    "error_freeze_path: $ErrorFreezePath",
    "error_freeze_sha256: $ErrorFreezeHash",
    "fix_note_path: $FixNotePath",
...
Write-Host '=== ROOT HELD GROUP ROUTE DRY-RUN SELECTOR V0.2 COMPLETE ==='
